...
/* --------------------------------------------------------------------------*/
int TwoPunctures::Index(int ivar, int i, int j, int k, int nvar, int n1, int n2,
                        int n3) const
{
    int i1 = i, j1 = j, k1 = k;
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::allocate_derivs(derivs *v, int n) const
{
    int m = n - 1;
    (*v).d0 = dvector(0, m);
    (*v).d1 = dvector(0, m);
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::free_derivs(derivs *v, int n) const
{
    int m = n - 1;
                          ...
                          derivs u)
{
    ...
    if (use_sources)
    {
        ...
        // @TODO: Set_Rho_ADM is an external call to another Cactuss thorn or
        // so.
        /* Set_Rho_ADM(cctkGH, n1*n2*n3, sources, s_x, s_y, s_z); */ // COMMENTED
                                                                     // AS WE DO
                                                                     // NOT HAVE
                                                                     // CCTK
                                                                     // HERE
            ...
            for (k = 0; k < n3; k++)
            {
                ...
                if (debugfile && (k == 0))
                {
                    ...
                    fprintf(
                        ...
                        (double)(-(2.0 * Pi / psi2 / psi * sources[indx]) *
                                 FAC),
                        (double)sources[indx]
                        /*(double)F[indx]*/
                    );
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::J_times_dv(int nvar, int n1, int n2, int n3, derivs dv,
                              double *Jdv, derivs u)
{ /*      Calculates the left hand sides of the non-linear equations
     F_m(v_n)=0*/
    /*      and the function u (u.d0[]) as well as its derivatives*/
...
{ /* Calculates rows of the vector 'J(FD)*dv'.*/
    ...
    double dV0, dV1, dV2, dV3, dV11, dV12, dV13, dV22, dV23, dV33, ha, ga, ga2,
                        ...
                        for (j1 = j_0; j1 <= j_1; j1++)
                        {
                            for (k1 = k_0; k1 <= k_1; k1++)
                            {
                                ...
                                for (ivar1 = 0; ivar1 < nvar; ivar1++)
                                {
                                    if (values[ivar1] != 0)
                                    {
                                        row = Index(ivar1, i1, j1, k1, nvar, n1,
                                                    n2, n3);
                                        mcol = ncols[row];
...
double TwoPunctures::PunctEvalAtArbitPosition(double *v, int ivar, double A,
                                              double B, double phi, int nvar,
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::calculate_derivs(int i, int j, int k, int ivar, int nvar,
                                    int n1, int n2, int n3, derivs v,
                                    derivs vv) const
{
...
/* --------------------------------------------------------------------------*/
double TwoPunctures::interpol(double a, double b, double c, derivs v) const
{
...
double TwoPunctures::PunctTaylorExpandAtArbitPosition(int ivar, int nvar,
                                                      int n1, int n2, int n3,
                                                      derivs v, double x,
 ...
 * routine */
double TwoPunctures::PunctEvalAtArbitPositionFast(double *v, int ivar, double A,
                                                  double B, double phi,
                                                  int nvar, int n1, int n2,
                                                  int n3) const
{
    int i, j, k, N;
    double *p, *values1, **values2, result;
...
/* --------------------------------------------------------------------------*/
double TwoPunctures::PunctIntPolAtArbitPositionFast(int ivar, int nvar, int n1,
                                                    int n2, int n3, derivs v,
                                                    double x, double y,
                                                    double z) const
{
...
void TwoPunctures::SpecCoef(int n1, int n2, int n3, int ivar, double *v,
                            double *cf)
{
