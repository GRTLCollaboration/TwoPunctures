...
/* --------------------------------------------------------------------------*/
double TwoPunctures::norm_inf(double const *TP_RESTRICT const F,
                              int const ntotal)
{
    double dmax = -1;
#pragma omp parallel
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::resid(double *TP_RESTRICT const res, int const ntotal,
...
/* -------------------------------------------------------------------------*/
void TwoPunctures::LineRelax_al(
    double *TP_RESTRICT const dv, int const j, int const k, int const nvar,
    int const n1, int const n2, int const n3,
    double const *TP_RESTRICT const rhs, int const *TP_RESTRICT const ncols,
    int const *TP_RESTRICT const *TP_RESTRICT const cols,
    double const *TP_RESTRICT const *TP_RESTRICT const JFD)
{
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::LineRelax_be(
    double *TP_RESTRICT const dv, int const i, int const k, int const nvar,
    int const n1, int const n2, int const n3,
    double const *TP_RESTRICT const rhs, int const *TP_RESTRICT const ncols,
    int const *TP_RESTRICT const *TP_RESTRICT const cols,
    double const *TP_RESTRICT const *TP_RESTRICT const JFD)
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::relax(double *TP_RESTRICT const dv, int const nvar,
                         int const n1, int const n2, int const n3,
                         double const *TP_RESTRICT const rhs,
                         int const *TP_RESTRICT const ncols,
                         int const *TP_RESTRICT const *TP_RESTRICT const cols,
...
/* --------------------------------------------------------------------------*/
void TwoPunctures::TestRelax(int nvar, int n1, int n2, int n3, derivs v,
                             double *dv)
{
    ...
    TP_INFO("Before: |F|=%20.15e", (double)norm1(res, ntotal));
    fflush(stdout);
    for (j = 0; j < NRELAX; j++)
    {
        ...
        {
            resid(res, ntotal, dv, F, ncols, cols, JFD);
            TP_INFO("j=%d\t |F|=%20.15e", j, (double)norm1(res, ntotal));
            fflush(stdout);
        }
    }

    resid(res, ntotal, dv, F, ncols, cols, JFD);
    TP_INFO("After: |F|=%20.15e", (double)norm1(res, ntotal));
    fflush(stdout);
...
/* --------------------------------------------------------------------------*/
int TwoPunctures::bicgstab(int const nvar, int const n1, int const n2,
                           int const n3, derivs v, derivs dv, int const output,
                           int const itmax, double const tol,
                           double *TP_RESTRICT const normres)
{
    ...
    if (output == 1)
    {
        TP_INFO("bicgstab:  itmax %d, tol %e", itmax, (double)tol);
        fflush(stdout);
    }
...
#pragma omp parallel for
    ...
    *normres = norm2(r, ntotal);
    if (output == 1)
    {
        TP_INFO("bicgstab: %5d  %10.3e", 0, (double)*normres);
        fflush(stdout);
    }
...
#pragma omp parallel for
            ...
            if (output == 1)
            {
                TP_INFO("bicgstab: %5d  %10.3e  %10.3e  %10.3e  %10.3e", ii + 1,
...
/* -------------------------------------------------------------------*/
void TwoPunctures::Newton(int const nvar, int const n1, int const n2,
                          int const n3, derivs v, double const tol,
                          int const itmax)
{
...
#pragma omp parallel for
        ...
        if (verbose == 1)
        {
            TP_INFO("Newton: it=%d \t |F|=%e", it, (double)dmax);
            TP_INFO("bare mass: mp=%g \t mm=%g", (double)par_m_plus,
                    (double)par_m_minus);
        }

        fflush(stdout);
        ii = bicgstab(nvar, n1, n2, n3, v, dv, verbose, 100, dmax * 1.e-3,
                      &normres);
#pragma omp parallel for
        for (int j = 0; j < ntotal; j++)
            v.d0[j] -= dv.d0[j];
